                ...
                value.title ?
                    <Menu.ItemGroup key={`${Menuitems.pathname}-${key}`} title={<span style={{color : 'white' , fontSize : '1rem', fontWeight : 'normal'  , padding : '3px 6px 6px 6px' , borderRadius : '10px'}}>{value.title}</span>}>
                        {value.list.map((v , k) =>{
                            return(
                                v.type === 'menu' ?
                                    <Menu.Item key={`${v.menu_name}`} icon={<Icon component={v.icon} />}>
                                    ...
                                    <SubMenu key={`${v.menu_name}`} icon={<Icon component={v.icon} />} title={v.title}>
                                        {v.menu_list.map((_v,_k) =>{
                                            return (
                                                _v.type === 'menu' &&
                                                <Menu.Item icon={<Icon component={_v.icon} />} key={`${_v.menu_name}`}><Link href={`/${Menuitems.pathname}/${_v.menu_name}`}>{_v.title}</Link></Menu.Item>
                                            )
                                        })
                                        }
                                    </SubMenu>
                    ...
                    <>
                        {value.list.map((v , k) =>{
                            return(
                                v.type === 'menu' ?
                                    <Menu.Item key={`${v.menu_name}`} icon={<Icon component={PictureIcon} />}>
                                        <Link href={`/${Menuitems.pathname}/${v.menu_name}`}>{v.title}</Link>
                                    </Menu.Item>
                                    :
                                    <SubMenu key={`${v.menu_name}`} icon={<Icon component={v.icon} />} title={v.title}>
