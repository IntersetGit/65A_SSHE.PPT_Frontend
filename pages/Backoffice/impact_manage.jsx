...
import { impact_data } from '../../config/impact_data';
...
const Impact = (props) => {
    ...
    const AddImpact = (type, _data = {}) => {
        ...
        switch (type) {
          case "ADD":
            console.log([...impact , {key : impact.length + 1 , id : `Imp${impact.length + 1}` , status : "Active" , ..._data}])
            setimpact([...impact , {key : impact.length + 1 , id : `Imp${impact.length + 1}`, status : "Active" , ..._data}])
            break;
    ...
    return(
        ...
        <Button type='primary' style={{ float : 'right' }} icon={<PlusOutlined />} onClick={() => showModal(1)} >เพิ่ม Impact</Button>
            <Table 
            columns={columns}
            dataSource={impact}
