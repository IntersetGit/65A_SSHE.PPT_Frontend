...
import { mitigration_data } from '../../config/mitigration_data';
...
const Mitigration = (props) => {
    ...
    const AddMitigration = (type, _data = {}) => {
        ...
        switch (type) {
          case "ADD":
            console.log([...mitigration , {key : mitigration.length + 1 , id : `Mit${mitigration.length + 1}` , status : "Active" , ..._data}])
            setmitigration([...mitigration , {key : mitigration.length + 1 , id : `Mit${mitigration.length + 1}`, status : "Active" , ..._data}])
            break;
    ...
    return(
        ...
        <Button type='primary' style={{ float : 'right' }} icon={<PlusOutlined />} onClick={() => showModal(1)} >เพิ่ม Mitigration</Button>
            <Table 
            columns={columns}
